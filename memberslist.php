 ...
 * -----------------------------
 *
 * $Id: memberslist.php 394 2006-12-28 10:24:34Z zanix $
 *
 ******************************/
...
if ('hide' == $roster_conf['hspvp_list_disp']) {
...
if (1 == $roster_conf['index_hslist']) {
    echo '    <td valign="top">';

    require_once  ROSTER_BASE . 'hslist.php';

    echo "    </td>\n";
}

if (1 == $roster_conf['index_pvplist']) {
    echo '    <td valign="top">';

    require_once  ROSTER_BASE . 'pvplist.php';
...
while ($row = $wowdb->fetch_assoc($result)) {
    ...
    foreach ($FIELDS as $field => $DATA) {
        ...
        if (1 == $roster_conf['index_tradeskill_icon'] && $field == $roster_conf['index_tradeskill_loc']) {
...
if ($roster_conf['index_update_inst']) {
    ...
    if (1 == $roster_conf['pvp_log_allow']) {
...
function last_up_value($row)
{
    ...
    if ('' != $row['last_update']) {
        ...
        [$month, $day, $year, $hour, $minute, $second] = sscanf($cell_value, '%d/%d/%d %d:%d:%d');
...
function class_value($row)
{
    ...
    if ('' != $row['class']) {
        ...
        if (1 == $roster_conf['index_class_color']) {
            foreach ($roster_conf['multilanguages'] as $language) {
                $class_color = array_search($row['class'], $wordings[$language], true);

                if (mb_strlen($class_color) > 0) {
...
function level_value($row)
{
    ...
    if (!empty($row['exp'])) {
        [$current, $max, $rested] = explode(':', $row['exp']);
