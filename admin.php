 ...
 * -----------------------------
 *
 * $Id: admin.php 364 2006-12-19 10:03:13Z zanix $
 ******************************/
...
if (!$roster_login->getAuthorized()) {
    ...
    print '<span class="title_text">' . $wordings[$roster_conf['roster_lang']]['roster_config'] . '</span><br>' .
        ...
        switch ($input_type[0]) {
            ...
            case 'radio':
                ...
                foreach ($options as $value) {
                    ...
                    $input_field .= '<label class="' . ($values['value'] == $vals[1] ? 'blue' : 'white') . '"><input class="checkBox" type="radio" name="config_' . $values['name'] . '" value="' . $vals[1] . '" ' . ($values['value'] == $vals[1] ? 'checked' : '') . '>' . $vals[0] . "</label>\n";
                }
                break;
            case 'select':
                $options = explode('|', $input_type[1]);
...
<tr>
	...
	<td class="membersRow' . (($i % 2) + 1) . '">';
            ...
            $html .= '<label class="' . ('1' == $values[$val_name] ? 'blue' : 'white') . '"><input class="checkBox" type="radio" name="disp_' . $values['member_id'] . ':' . $val_name . '" value="1" ' . ('1' == $values[$val_name] ? 'checked' : '') . '>off</label><br>' . "\n";

            $html .= '<label class="' . ('3' == $values[$val_name] ? 'blue' : 'white') . '"><input class="checkBox" type="radio" name="disp_' . $values['member_id'] . ':' . $val_name . '" value="3" ' . ('3' == $values[$val_name] ? 'checked' : '') . '>on</label>' . "\n";
...
function processData()
{
    ...
    foreach ($_POST as $settingName => $settingValue) {
        ...
        if ('config_' == mb_substr($settingName, 0, 7)) {
            $settingName = str_replace('config_', '', $settingName);

            // Fix directories

            if ('img_url' == $settingName || 'interface_url' == $settingName) {
                ...
                if ('/' != mb_substr($settingValue, -1, 1)) {
                    $settingValue .= '/';
                }

                if ('/' == mb_substr($settingValue, 0, 1)) {
                    $settingValue = mb_substr($settingValue, 1);
                }
            }

            // Fix roster url

            if ('roster_dir' == $settingName) {
                // Replace back-slashes
                ...
                if ('/' == mb_substr($settingValue, -1, 1)) {
                    $settingValue = mb_substr($settingValue, 0, -1);
                }
            }
            ...
            if ($config[$settingName] != $settingValue && 'process' != $settingName) {
 ...
 *
 * @return string on success, error string on failure
 */
