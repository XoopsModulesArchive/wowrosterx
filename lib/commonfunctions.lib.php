 ...
 * -----------------------------
 *
 * $Id: commonfunctions.lib.php 364 2006-12-19 10:03:13Z zanix $
 *
 ******************************/
 ...
 * @param null   $header_text Place text in a styled header
 * @return string
 */
function border($style, $mode, $header_text = null)
{
    ...
    if ('s' == mb_substr($style, 0, 1)) {
        $style = 'simple';
    }

    $style .= 'border';

    if ('' != $header_text && 'end' != $style) {
        $header_text = '  <tr>
...
function sql_highlight($sql)
{
    ...
    $red_keywords = ['/(INSERT INTO)/', '/(UPDATE\s+)/', '/(DELETE FROM\s+)/', '/(CREATE TABLE)/', '/(IF (NOT)? EXISTS)/',
                          '/(ALTER TABLE)/', '/(CHANGE)/', '/(SET)/', '/(REPLACE INTO)/', ];

    $red_replace = array_fill(0, count($red_keywords), '<span class="red">\\1</span>');

    $sql = preg_replace($red_keywords, $red_replace, $sql);

    // Passive keywords

    $green_keywords = ['/(SELECT)/', '/(FROM)/', '/(WHERE)/', '/(LIMIT)/', '/(ORDER BY)/', '/(GROUP BY)/',
                            '/(\s+AND\s+)/', '/(\s+OR\s+)/', '/(\s+ON\s+)/', '/(BETWEEN)/', '/(DESC)/', '/(LEFT JOIN)/', '/(SHOW TABLES)/',
                            '/(LIKE)/', '/(PRIMARY KEY)/', '/(VALUES)/', '/(TYPE)/', '/(ENGINE)/', '/(MyISAM)/', '/(SHOW COLUMNS)/', ];

    $green_replace = array_fill(0, count($green_keywords), '<span class="green">\\1</span>');
...
{
    if (false === ($extension = pathinfo($imagefilename, PATHINFO_EXTENSION))) {
        return false;
    }  
 ...
 * @param int    $inline_caption | (optional)
 *                               Default is true
    ...
    foreach (explode("\n", $tooltip) as $line) {
        ...
        if (!empty($line)) {
            ...
            if ($first_line) {
                ...
                if (mb_strlen($caption_color) > 6) {
                    $color = mb_substr($caption_color, 2, 6) . ';font-size:12px;font-weight:bold';
                } else {
                ...
                if (preg_match('^' . $wordings[$locale]['tooltip_use'], $line)) {
                    $color = '00ff00';
                } elseif (preg_match('^' . $wordings[$locale]['tooltip_requires'], $line)) {
                    $color = 'ff0000';
                } elseif (preg_match('^' . $wordings[$locale]['tooltip_reinforced'], $line)) {
                    $color = '00ff00';
                } elseif (preg_match('^' . $wordings[$locale]['tooltip_equip'], $line)) {
                    $color = '00ff00';
                } elseif (preg_match('^' . $wordings[$locale]['tooltip_chance'], $line)) {
                    $color = '00ff00';
                } elseif (preg_match('^' . $wordings[$locale]['tooltip_enchant'], $line)) {
                    $color = '00ff00';
                } elseif (preg_match('^' . $wordings[$locale]['tooltip_soulbound'], $line)) {
                    $color = '00bbff';
                } elseif (preg_match('^' . $wordings[$locale]['tooltip_set'], $line)) {
                    $color = '00ff00';
...
function makeOverlib($tooltip, $caption = '', $caption_color = '', $mode = 0, $locale = '', $extra_parameters = '')
{
    ...
    if ('' != $caption_color) {
        if (mb_strlen($caption_color) > 6) {
            $caption_color = mb_substr($caption_color, 2);
        }
    }
 ...
 * @return string | The HTML for the messagebox
 */
 ...
 * @param string $title | The box title
 * @param string $style | The border style
 * @param bool $open | True if initially open
 * @param string $width | Initial width with unit
 * @return string | The HTML for the messagebox
 */
