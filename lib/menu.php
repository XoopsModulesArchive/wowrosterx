 ...
 * -----------------------------
 *
 * $Id: menu.php 390 2006-12-27 08:58:40Z zanix $
 *
 ******************************/
...
if ($guild_data && $guild_data_rows > 0) {
    ...
    while (false !== ($row = $wowdb->fetch_assoc($result_menu))) {
        if ($row['isalt']) {
            $num_alts += $row['amount'];
        } else {
...
if ($roster_conf['menu_right_pane'] && $guild_data_rows > 0) {
    ...
    if ($roster_conf['rs_mode']) {
        ...
        require_once(ROSTER_BASE . 'realmstatus.php');
    } else {
        print '&nbsp;</td>';
    }
}
...
function DateDataUpdated($updateTimeUTC)
{
    ...
    [$month, $day, $year, $hour, $minute, $second] = sscanf($updateTimeUTC, '%d/%d/%d %d:%d:%d');
    ...
    if ('' != count($addons)) {
        foreach ($addons as $addon) {
            ...
            if (file_exists($menufile)) {
                ...
                if ($config['menu_min_user_level'] <= 0) { //modify this line for user level / authentication stuff (show the link for user level whatever for this addon)  you understand :P
                    if (isset($config['menu_index_file'][0])) {
                        //$config['menu_index_file'] is the new array type

                        foreach ($config['menu_index_file'] as $addonLink) {
                            $fullQuery = '?roster_addon_name=' . urlencode($addon) . ($addonLink[0] ?? '');
