 ...
 * -----------------------------
 *
 * $Id: pvp3.php 341 2006-12-10 06:20:26Z zanix $
 *
 ******************************/
...
function output_pvp_summary($pvps, $type)
{
    ...
    foreach ($pvps as $row) {
        if ('1' == $row->data['win']) {
            ...
            $ave_win_level_diff += $row->data['leveldiff'];
        } else {
            $tot_losses += 1;

            $ave_loss_level_diff += $row->data['leveldiff'];
        }
    }

    if ($tot_wins > 0) {
        $ave_win_level_diff /= $tot_wins;
    ...
    if ($tot_losses > 0) {
        $ave_loss_level_diff /= $tot_losses;
...
function output_bglog($member_id)
{
    ...
    foreach ($bg_array as $bgname) {
        ...
        $worst = $subs[count($subs) - 1]['Zone'];

        $bestNum = $subs[0]['WinLoss'];

        $worstNum = $subs[count($subs) - 1]['WinLoss'];
...
function output_duellog($member_id)
{
    ...
    foreach ($data as $datakey => $dataset) {
        ...
        foreach ($roster_conf['multilanguages'] as $language) {
            ...
            if (mb_strlen($dataset['icon_name']) > 0) {
                break;
            }
        }
        ...
        $check_array = ['name', 'guild', 'race', 'class', 'leveldiff', 'countn'];
