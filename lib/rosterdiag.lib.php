 ...
 * -----------------------------
 *
 * $Id: rosterdiag.lib.php 348 2006-12-11 00:28:00Z mathos $
 *
 ******************************/
...
$ignored_dirs = ['.', '..', 'SVN', '.svn', 'Interface'];
// Files to check with extension:
//$extensions = array('php', 'css', 'js', 'tpl', 'htm', 'html');
$extensions = ['php', 'inc', 'css', 'js', 'tpl', 'htm', 'html', 'jpg', 'gif', 'png', 'sql', 'txt'];
// Files to ignore
$ignored_files = ['conf.php', '.htaccess'];
...
$severity[0] = ['style' => 'sgreen', 'color' => '#12C312', 'weight' => 0, 'severityname' => 'No Issues'];
$severity[1] = ['style' => 'sgray', 'color' => '#AFAFAF', 'weight' => 0, 'severityname' => 'Unknown'];
$severity[2] = ['style' => 'sblue', 'color' => '#312CF8', 'weight' => 1, 'severityname' => 'Initial'];
$severity[3] = ['style' => 'spurple', 'color' => '#E920CF', 'weight' => 1, 'severityname' => 'Strange'];
$severity[4] = ['style' => 'syellow', 'color' => '#F1B10E', 'weight' => 3, 'severityname' => 'Minor'];
$severity[5] = ['style' => 'sorange', 'color' => '#EE870D', 'weight' => 7, 'severityname' => 'Major'];
$severity[6] = ['style' => 'sred', 'color' => '#FF0000', 'weight' => 15, 'severityname' => 'Critical'];

$rollups[] = ['rollup' => 2, 'severity' => 4];
$rollups[] = ['rollup' => 5, 'severity' => 5];
$rollups[] = ['rollup' => 14, 'severity' => 6];
...
$directories = ['.' => ['localfiles' => 0, 'remotefiles' => 0, 'severity' => 0]];
    ...
    if ($handle = opendir($dir)) {
        while ($filename = readdir($handle)) {
            ...
            if (is_dir($directory) && CheckDirectory($filename)) {
                $directories[$directory] = ['localfiles' => 0, 'remotefiles' => 0, 'severity' => 0];
...
function CheckExtension($filename)
{
    ...
    if ('.' == $filename || '..' == $filename || 'version_match.php' == $filename) {
        $returnvalue = 0;
    } else {
        // Check the extension
        ...
        if (in_array($fileextension, $extensions, true) && !in_array($filename, $ignored_files, true)) {
            $returnvalue = 1;
        } else {
            $returnvalue = 0;
        }
...
function GetFileVersionInfo($directory, $file)
{
    ...
    if (check_if_image($file)) {
        ...
        // String to match in SVN: $Id: rosterdiag.lib.php 348 2006-12-11 00:28:00Z mathos $
        ...
        if ((preg_match('~\s\$Id\:\s(.+?)\s(.+?)\s(.+?)\s(.+?)\s(.+?)\s\$~', $fileheader, $local_version) > 0) || (preg_match('~\s\$Id\:\s(.+?)\,v\s(.+?)\s(.+?)\s(.+?)\s(.+?)\sExp\s\$~', $fileheader, $local_version) > 0)) {
            ...
            if (1 == preg_match('~\$versions\[\'versionDate\'\]\[\'(.+?)\'\]\s\=\s\'\$Date\:\s(.+?)\s\$\'\;~', $fileheader, $local_version)) {
                $files[$directory][$file]['local']['versionDesc'] = $local_version[1];
            ...
            if (1 == preg_match('~\$versions\[\'versionRev\'\]\[\'(.+?)\'\]\s\=\s\'\$Revision\:\s(.+?)\s\$\'\;~', $fileheader, $local_version)) {
                $files[$directory][$file]['local']['versionRev'] = $local_version[2];
            } else {
            ...
            if (1 == preg_match('~\$versions\[\'versionAuthor\'\]\[\'(.+?)\'\]\s\=\s\'\$Author\:\s(.+?)\$\'\;~', $fileheader, $local_version)) {
        ...
        foreach ($filedata as $filename => $file) {
            ...
            if (isset($file['local']) && isset($file['remote'])) {
                ...
                if (version_compare($file['local']['versionRev'], $file['remote']['versionRev']) < 0) {
                    ...
                    $files[$directory][$filename]['diff'] = 1;
                } elseif (0 == version_compare($file['local']['versionRev'], $file['remote']['versionRev'])) {
...
function ConfigErrors()
{
    ...
    if (1 == $roster_conf['motd_display_mode']) {
        if (!function_exists('gd_info')) {
...
function downloadsvn($filename)
{
    ...
    while (!feof($rh)) {
        if (false === fwrite($wh, fread($rh, 1024))) {
