 ...
 * -----------------------------
 *
 * $Id: membersinst.php 394 2006-12-28 10:24:34Z zanix $
 *
 ******************************/
...
function tableHeaderRow($th)
{
    ...
    foreach ($th as $header) {
        ...
        if ($items[$header]) {
            [$iname, $thottnum] = explode('|', $items[$header][$header]);
...
$items = $inst_keys[$roster_conf['roster_lang']][ mb_substr($faction, 0, 1) ];
$keys = ['Name'];
...
while (false !== ($row = $wowdb->fetch_array($result))) {
    if ('' == $row['clientLocale']) {
        $row['clientLocale'] = $roster_conf['roster_lang'];
    }

    $items = $inst_keys[$row['clientLocale']][ mb_substr($faction, 0, 1) ];
    ...
    foreach ($items as $key => $item) {
        foreach ($items[$key] as $subkey => $subitem) {
            $onechar = mb_substr($subkey, 0, 1);

            if (!is_numeric($onechar)) {
                ++$countk;

                [$iname, $thottnum] = explode('|', $subitem);
    ...
    foreach ($items as $key => $item) {
        if ('-1' == $krow[$key]) {
            ...
            if ($row['class'] == $wordings[$row['clientLocale']]['Rogue']) {
                ...
                [$current_skill, $max_skill] = explode(':', $srow['skill_level']);
            ...
            if ('Quests' == $items[$key][0]) {
                $type = 'quest';
            } elseif ('Parts' == $items[$key][0]) {
                $type = 'item';
            } else {
                continue;
            }
    ...
    foreach ($items as $key => $data) {
        ...
        if ('-2' == $krow[$key]) {
            ...
            [$iname, $thottnum] = explode('|', $data[$key]);
            ...
            [$iname, $thottnum] = explode('|', $items[$key][$key]);

            $qcount = count($items[$key]) - 2;    //number of parts/quests
            if ('Quests' == $items[$key][0]) {    //-> $krow[$key] = "5" (e.g.)
                $bcount = $krow[$key];
            ...
            if ('Quests' == $items[$key][0]) {
                for ($i = 1; $i < count($items[$key]) - 1; $i++) {
                    ...
                    [$qname, $junk] = explode('|', $items[$key][$i]);
                ...
                for ($i = 1; $i < count($items[$key]) - 1; $i++) {
                    ...
                    [$pname, $junk] = explode('|', $items[$key][$i]);
