...
* -----------------------------
*
 * $Id: rosterdiag.php 348 2006-12-11 00:28:00Z mathos $
*
******************************/
...
if (isset($_GET['printconf']) && 1 == $_GET['printconf']) {
...
if (isset($_POST['filename']) && isset($_POST['downloadsvn'])) {
    if ('confirmation' == $_POST['downloadsvn']) {
        ...
        if (isset($_POST['downmode']) && 'install' == $_POST['downmode']) {
...
<table width="300" class="bodyline" cellspacing="0">
	...
	<tr>
		...
		<td class="membersRowRight' . ((($rowstripe) % 2) + 1) . '">' . $GLOBALS['xoopsDB']->getServerVersion() . '</td>
	</tr>
</table>' .
border('syellow', 'end') . '
<br>
...
if (ini_get('allow_url_fopen')) {
    ...
    if ($roster_login->getAuthorized()) {
        ...
        foreach ($directories as $directory => $filecount) {
            if (isset($files[$directory])) {
                foreach ($files[$directory] as $file => $filedata) {
                    if ($filedata['update']) {
                        if (isset($file) && 'severity' != $file && 'tooltip' != $file && 'rollup' != $file && 'rev' != $file && 'date' != $file && 'author' != $file && 'md5' != $file && 'update' != $file && 'missing' != $file) {
                            if ('' != $zippackage_files) {
                                $zippackage_files .= ';';
                            }
        ...
        if ('' != $zippackage_files) {
            ...
            echo '<input type="radio" name="ziptype" value="zip" checked>.zip Archive<br>';
    ...
    require_once('rosterdiag_sql.php');
    ...
    foreach ($directories as $directory => $filecount) {
        if (isset($files[$directory])) {
            ...
            $directory_id = str_replace(['.', '/', '\\'], '', $directory);
            ...
            foreach ($files[$directory] as $file => $filedata) {
                ...
                if (isset($file) && 'severity' != $file && 'tooltip' != $file && 'rollup' != $file && 'rev' != $file && 'date' != $file && 'author' != $file && 'md5' != $file && 'update' != $file && 'diff' != $file && 'missing' != $file) {
