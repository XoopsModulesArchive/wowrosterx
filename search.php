 ...
 * -----------------------------
 *
 * $Id: search.php 394 2006-12-28 10:24:34Z zanix $
 *
 ******************************/
...
<div style="color:white;text-align;center">
  ...
  <tr>
    ...
    if (0 != $wowdb->num_rows($result)) {
        $cid = '';

        $rc = 0;

        while (false !== ($data = $wowdb->fetch_assoc($result))) {
            $row_st = (($rc % 2) + 1);
            ...
            foreach (explode("\n", $data['item_tooltip']) as $line) {
                ...
                if (!empty($line)) {
                    ...
                    if ($first_line) {
                        ...
                        if (mb_strlen($data['item_color']) > 6) {
                            $color = mb_substr($data['item_color'], 2, 6) . ';font-size:12px;font-weight:bold';
                        } else {
