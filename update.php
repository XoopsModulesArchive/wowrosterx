 ...
 * -----------------------------
 *
 * $Id: update.php 371 2006-12-23 11:40:07Z zanix $
 *
 ******************************/
...
if ($roster_conf['use_update_triggers']) {
    require_once  ROSTER_LIB . 'update_trigger_lib.php';
}
...
if (is_array($_FILES) && !empty($_FILES)) {
    ...
    foreach ($_FILES as $filefield => $file) {
        if (!empty($file['name'])) {
            ...
            [$file_name, $file_ext, $file_type] = explode('.', $file['name']);

            if (in_array(mb_strtolower($file_name), $filefields, true)) {
    ...
    if (!$roster_conf['authenticated_user']) {
        ...
        if ($uploadFound) {
            ...
            if (!empty($errorstringout)) {
                ...
                print '<input type="hidden" name="data" value="' . htmlspecialchars(stripAllHtml($errorstringout), ENT_QUOTES | ENT_HTML5) . '">' . "\n";
            ...
            print '<input type="hidden" name="data" value="' . htmlspecialchars(stripAllHtml($updateMessages . $updatePvPMessages . $rosterUpdateMessages), ENT_QUOTES | ENT_HTML5) . '">' . "\n";
