 ...
 * -----------------------------
 *
 * $Id: memberdetails.php 394 2006-12-28 10:24:34Z zanix $
 *
 ******************************/
...
if (0 == $roster_conf['server_name_comp'] && isset($_GET['server'])) {
...
foreach ($disp_array as $global_setting => $user_setting) {
    if ('2' == $roster_conf[$global_setting]) {
...
switch ($action) {
    case 'character':
        ...
        break;
    case 'bags':
        if (1 == $roster_conf['show_inventory']) {
    ...
    case 'bank':
        ...
        break;
    case 'quests':
        if (1 == $roster_conf['show_quests']) {
            echo $char->show_quests();
        }
        break;
    case 'recipes':
        if (1 == $roster_conf['show_recipes']) {
            print $char->show_recipes();
        }
        break;
    case 'bg':
        if (1 == $roster_conf['show_bg']) {
            $url .= '&amp;action=bg';
    ...
    case 'duels':
        ...
        break;
    case 'spellbook':
        if (1 == $roster_conf['show_spellbook']) {
...
if ('character' == $action && $roster_conf['show_item_bonuses']) {
